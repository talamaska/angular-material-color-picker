(function (root, factory) {
    ...
    function ColorPickerController($element, ColorPickerService) {
        ...
        function onRotatorDrag(angle) {
            var roundedAngle = parseInt(angle, 10);
            var saturation = 100;
            var luminosity = 50;
            var alpha = 1;
            var color = 'hsla(' + roundedAngle + ', ' + saturation + '%, ' + luminosity + '%, ' + alpha + ')';
            var rgbModel = ColorPickerService.hslToRgb(roundedAngle, saturation, luminosity);
            var hexModel = ColorPickerService.hslToHex(roundedAngle, saturation, luminosity);
        ...
        function $onChanges() {
            if (this.color) {
                this.angle = ColorPickerService.getHueFromRgb(this.color.red, this.color.green, this.color.blue);
            }
        }
